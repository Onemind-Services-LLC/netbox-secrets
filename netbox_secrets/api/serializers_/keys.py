...
class SessionKeyCreateSerializer(serializers.ModelSerializer):
    ...
    private_key = serializers.CharField(
        write_only=True,
        required=True,
        help_text="User's RSA private key in PEM format"
    )
...
class RSAKeyPairSerializer(serializers.Serializer):
    ...
    private_key = serializers.CharField(
        help_text="RSA private key in PEM format (keep secure!)"
    )
...
class ActivateUserKeySerializer(serializers.Serializer):
    ...
    private_key = serializers.CharField(
        write_only=True,
        required=True,
        help_text="Administrator's RSA private key for deriving master key"
    )
...
class UserKeySerializer(NetBoxModelSerializer):
    ...
    private_key = serializers.CharField(
        write_only=True,
        required=False,
        help_text="RSA private key in PEM format (write-only, used for activation)"
    )
    is_active = serializers.BooleanField(
        read_only=True,
        help_text="Whether this key has been activated with the master key"
    )
    is_filled = serializers.BooleanField(
        read_only=True,
        help_text="Whether this key has a public key configured"
    )
...
class SessionKeySerializer(serializers.ModelSerializer):
    ...
    url = serializers.HyperlinkedIdentityField(
        view_name='plugins-api:netbox_secrets-api:sessionkey-detail'
    )
    userkey = UserKeySerializer(nested=True, read_only=True)
    session_key = serializers.SerializerMethodField(
        read_only=True,
        help_text="Base64-encoded session key (only returned on creation)"
    )
